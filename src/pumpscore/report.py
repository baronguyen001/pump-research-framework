...
def render_markdown(
    ...
    *,
    ...
    """Render the scorecard as a Markdown document. Pure, offline, deterministic.

    Handy for pasting into a GitHub issue, a Discord message, or a research note
    where a full standalone HTML page would be overkill.
    """
    result = score(card, weights)
    total = cast(float, result["total"])
    band = band_for(total)
    ...
    lines = [
        f"# pumpscore - {card.token}",
        "",
        f"**{total:g}/100 - {band.label}**",
        "",
        f"> {band.action}",
        "",
        "| Layer | Points |",
    ...
    for name in LAYER_NAMES:
        ...
        lines.append(f"| {name.title()} | {points:g} / 25 |")
    ...
    if checklist is not None:
        lines += [
            ...
            f"- Patterns hit: {checklist.patterns_hit}/10",
            f"- Red flags: {checklist.red_flags_hit}/7",
        ]
        if checklist.flagged:
            lines.append(f"- Flagged: {', '.join(checklist.flagged)}")

    if stage_signals is not None:
        stage = classify_stage(stage_signals)
        goldilocks = goldilocks_gate(card)
        lines += [
            "",
            "## Lifecycle",
            f"- Stage: **{stage}**",
            f"- Goldilocks gate: {'PASS' if goldilocks.passes else 'FAIL'}",
            f"- Anti-FOMO: {fomo_decision(card)}",
        ]

    lines += [
        "",
        "_Generated by pumpscore (offline, keyless). Educational only, not financial advice._",
        "",
    ]
    return "\n".join(lines)
...
def render_from_card_dict(
    ...
    Mirrors how the CLI parses a single scorecard file so the report reflects the
    same patterns, red flags, and stage signals as the text output. ``fmt`` may
    be ``"html"`` (default) or ``"markdown"``.
    """
